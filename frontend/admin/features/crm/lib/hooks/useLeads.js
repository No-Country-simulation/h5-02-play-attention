  ...
  return useMutation({
    mutationFn: async ({ leadId, status }) => {
      ...
      const payload = {
        ...
        newsletter: adaptedLead.newsletter || false // Preservar valor de newsletter
      };
...
/**
 * Hook para actualizar el campo newsletter de un lead
 * @returns {Object} Mutation para actualizar el campo newsletter
 */
export function useUpdateLeadNewsletter() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ leadId, newsletter }) => {
      // Validación estricta del ID
      if (!leadId || typeof leadId !== 'string' || leadId.trim() === '') {
        throw new Error(`ID del lead inválido: "${leadId}"`);
      }
      ...
      console.log(
        `🔄 Actualizando newsletter del lead ${leadId} a: ${newsletter}`
      );
      ...
      const payload = {
        ...
        origen: mapSourceToBackend(adaptedLead.source) || 'Sitio web',
        relation: adaptedLead.position || 'Usuario',
        newsletter: newsletter // Actualizar valor de newsletter
      };

      return leadsApi.updateLead(leadId, payload);
    },
    onSuccess: () => {
      toast.success('Suscripción del lead actualizada');
      queryClient.invalidateQueries({ queryKey: ['leads'] });
    },
    onError: error => {
