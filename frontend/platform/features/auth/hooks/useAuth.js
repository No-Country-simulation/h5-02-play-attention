...
export const useLogin = (redirectUrl = '/dashboard') => {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: data => {
      ...
      // Ahora SOLO los usuarios con rol 'user' pueden acceder
      if (userRole !== 'User' && userRole !== 'user') {
        // Mostrar modal de acceso denegado para los que NO son user
        setShowAccessDenied(true);
        return;
      }

      // Si el rol es user, continuar con el flujo normal
      ...
      // Redirigir a la URL específica (dashboard por defecto)
